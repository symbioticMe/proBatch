...
loess_fit <- adjust_batch_trend_df(quantile_normalized_long, example_sample_annotation)
...
loess_fit_30 <- adjust_batch_trend_df(quantile_normalized_long, example_sample_annotation, span = 0.3)
...
loess_fit_70 <- adjust_batch_trend_df(quantile_normalized_long, example_sample_annotation, span = 0.7)
plot_with_fitting_curve(feature_name = "10231_QDVDVWLWQQEGSSK_2", 
...
be passed as parameters to `adjust_batch_trend_df()`.
...
function `correct_batch_effects_df()` corrects MS signal drift and discrete
...
`abs_threshold` or `pct_threshold` as in `adjust_batch_trend_df()`.
...
batch_corrected_matrix = long_to_dm(batch_corrected_df)
```
