...
thoroughly tested on SWATH data. Thus, as an example dataset we include
...
To guarantee uniform color annotation, function `sample_annotation_to_colors()` 
...
```{r plot_PVCA, fig.show='hold', fig.width=5, fig.height=5}
...
mixture, one can use the `plot_spike_in()` function instead. In most
