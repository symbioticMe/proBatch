...
#TODO: check if "BioBase" needs to be added to bioc_deps
cran_deps <- c("corrplot", "data.table", "ggplot2", "ggfortify",
               "ggpubr", "grid", "gridExtra", "gtable", 
               "lazyeval", "pheatmap", "reshape2", "rlang", 
...
For batch correction, we also assume, that the imputed values, e.g. requant values from OpenSWATH output,
...
For the analysis, we also need `order_col`. This column is inferred from `RunDate` and `RunTime`,
creating a joined `DateTime` column. Although both `order` (the default name of `order_col`) 
and `DateTime` columns can be created with the package function `date_to_sample_order` 
(see Utility functions below), here they are provided already in the examples 
to allow user to skip this function, if `order` is defined already or not 
relevant/unknown for the specific dataset.

Thus, technical and biological factors are:
```{r tech_bio_cols}
technical_covariates = c('MS_batch', 'digestion_batch', 'DateTime')
biological_covariates = c('Strain', 'Diet', 'Sex')
...
different peptides in a batch-specific way), and continuous shifts from MS drift
...
```{r}
log_transformed_matrix <- log_transform_matrix(example_matrix, log_base = 2, offset = 1)
```
...
factor columns are mapped to qualitative colors (maximally distinct), while
date and numeric columns are mapped to sequential (gradient) colors.
...
median_normalized_matrix = normalize_data(example_matrix, 
                                          normalize_func = "medianCentering", log_base = 2)
```
...
```{r plot_PCA, fig.show='hold', fig.width=10, fig.height=11}
...
library(ggpubr)
ggarrange(pca1, pca2, pca3, ncol = 2, nrow = 2)
```
...
(peptide, protein, gene), the quantitative behavior of which is
...
loess_fit <- adjust_batch_trend(quantile_normalized_long, example_sample_annotation)
loess_fit_df <- loess_fit$corrected_df
```
...
loess_fit_30 <- adjust_batch_trend(quantile_normalized_long, example_sample_annotation, span = 0.3)

plot_with_fitting_curve(feature_name = "10231_QDVDVWLWQQEGSSK_2", 
            fit_df = loess_fit_30$fit_df, fit_value_var = 'fit',
            df_long = quantile_normalized_long, sample_annotation = example_sample_annotation, 
            color_by_batch = TRUE, color_scheme = color_list[[batch_col]], plot_title = "Span = 30%")
```

```{r loess_70, fig.show='hold', fig.width=5, fig.height=2.4}
loess_fit_70 <- adjust_batch_trend(quantile_normalized_long, example_sample_annotation, span = 0.7)
plot_with_fitting_curve(feature_name = "10231_QDVDVWLWQQEGSSK_2", 
            fit_df = loess_fit_70$fit_df, fit_value_var = 'fit',
            df_long = quantile_normalized_long, sample_annotation = example_sample_annotation, 
            color_by_batch = TRUE, color_scheme = color_list[[batch_col]],plot_title = "Span = 70%")
```
...
peptides, absolute and relative thresholds (`abs_threshold` and
`pct_threshold`) on the number of missing values for each peptide can
be passed as parameters to `adjust_batch_trend()`.
...
Currently, two methods of batch correction are implemented:
...
peptide_median_df <- center_feature_batch_medians(loess_fit_df, example_sample_annotation)
...
function `correct_with_ComBat_df()` can incorporate several covariates
...
plot_single_feature (feature_name = "10231_QDVDVWLWQQEGSSK_2", df_long = loess_fit_df, 
          sample_annotation = example_sample_annotation, plot_title = "Loess Fitted")
plot_single_feature (feature_name = "10231_QDVDVWLWQQEGSSK_2", df_long =  comBat_df, 
          sample_annotation = example_sample_annotation, plot_title = "ComBat corrected")
```
...
correction method is preferred at `discrete_func` either “ComBat” or
“MedianCentering” and supplement other arguments such as `span`,
`abs_threshold` or `pct_threshold` as in `normalize_custom_fit()`.

```{r batch_corr_general, fig.show='hold'}
batch_corrected_df <- correct_batch_effects(df_long = quantile_normalized_long,
                                            sample_annotation = example_sample_annotation, 
                                            discrete_func = 'ComBat', continuous_func = 'loess_regression',
...
```{r, fig.show='hold', fig.width=10, fig.height=11}
p1_exp = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'pheatmap')
p2_exp = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'corrplot', plot_title = 'Correlation of selected samples')
```
...
Aesthetically more pleasing version:
```{r, fig.show='hold', fig.width=10, fig.height=11}
p2_exp = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'corrplot', plot_title = 'Correlation of selected samples', tl.col = 'black', mar = c(0,0,5,0), number.cex=0.75, tl.cex = 0.8, cl.lim=c(0.7,1), upper.col=colorRampPalette(c("blue","white","lightpink", "red","brown"))(20), lower.col = colorRampPalette(c("blue","white","lightpink", "red","brown"))(20), is.corr = FALSE)
```
...
p1 = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'pheatmap', 
                         sample_annotation = example_sample_annotation %>% select(c(factors_to_show, 'FullRunName')), 
                         plot_title = 'Log transformed correlation matrix of selected replicated samples', 
...
p2 = plot_sample_corr_heatmap(batch_corrected_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'pheatmap', 
                         sample_annotation = example_sample_annotation %>% select(c(factors_to_show, 'FullRunName')), plot_title = 'Batch Corrected',
                         annotation_colors = color_list[factors_to_show], 
                         color = heatmap_colors, breaks = breaksList, 
                         cluster_rows=FALSE, cluster_cols=FALSE,fontsize = 4,
                         annotation_names_col = TRUE, annotation_legend = FALSE, 
...
```{r correlation_of_peptides, eval = FALSE}
peptide_cor_raw <- plot_peptide_corr_distribution(log_transformed_matrix, 
          generated_peptide_annotation, protein_col = 'Gene', plot_title = 'Peptide correlation (raw)')

peptide_cor_batchCor <- plot_peptide_corr_distribution(batch_corrected_matrix, 
          generated_peptide_annotation, protein_col = 'Gene', plot_title = 'Peptide correlation (batch corrected)')
...
study:
```{r, include = TRUE, fig.align = "center", echo=FALSE, fig.height=1.5, fig.width=4}
