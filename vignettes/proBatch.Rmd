...
To garantee uniform color annotation, function `sample_annotation_to_colors()` 
can be used. Using this function biological and technical factor columns are 
mapped to qualitative colors (maximally distinct), while
...
```{r plot_PCA, fig.show='hold', fig.width=10, fig.height=8.5}
...
Note that when the color scheme is not provided, `digestion_batch` is mapped to 
continuous scale. Which is why defining color scheme is so important.
```{r plot_PCA_spec, fig.show='hold', fig.width=5, fig.height=5}
...
p1_exp = plot_sample_corr_heatmap(log_transformed_matrix, 
                                  ...
                                  flavor = 'pheatmap')
p2_exp = plot_sample_corr_heatmap(log_transformed_matrix, 
                                  samples_to_plot = replicate_filenames, 
                                  flavor = 'corrplot', 
...
Aesthetically more pleasing version, specifying parameters for the `corrplot`:
```{r, fig.show='hold', fig.width=10, fig.height=11}
p2_exp = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames, 
                         flavor = 'corrplot', 
                         plot_title = 'Correlation of selected samples', 
                         tl.col = 'black', mar = c(0,0,5,0), number.cex=0.75, 
                         tl.cex = 0.8, cl.lim=c(0.7,1), 
                         upper.col=colorRampPalette(c('blue','white','lightpink', 'red','brown'))(20), 
                         lower.col = colorRampPalette(c('blue','white','lightpink', 'red','brown'))(20), 
                         is.corr = FALSE)
```
...
p1 = plot_sample_corr_heatmap(log_transformed_matrix, samples_to_plot = replicate_filenames,
                              flavor = 'pheatmap', 
                              sample_annotation = example_sample_annotation %>% 
                                select(c(factors_to_show, 'FullRunName')), 
