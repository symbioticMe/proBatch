...
abstract: |
  This vignette describes how to apply different functions from the proBatch package to diagnose and correct for batch effects. The functions are primarily intended for proteomic SWATH data that has been generated from OpenSWATH but can also be applied to other omics dataset. 
  
  The proBatch package provides a complete set of functions for batch correction workflow: filter peptides by requants or missing values, normalize global intensity, diagnose for proteome-wide or feature-level batch effects, correct for continuous drift or discrete batch effects and evaluate by correlation based quality control metrics. In particular for proteomics, we strongly recommend to normalize and batch-correct on peptide-level data generated from OpenSWATH, then proceed with protein quantity inference. 
...
## Batch effects in large-scale high throughput data 
Recent advances in mass-spectrometry enabled fast and near-exhaustive identification and quantification of proteins in complex biological samples [1]. Quantitative robustness and sample throughput of techniques such as DIA/SWATH-MS allow to profile large sample cohorts with high quantitative accuracy [2 -5]. However, consistent and accurate profiling for large-scale studies still remains challenging. Obtaining sufficiently large dataset is associated with considerable logistics efforts of multiple biomaterial handlers on sample preparation and data acquisition steps e.g. protein extraction, peptide digestion, instrument cleaning. This introduces systematic technical variation known as the batch effect.

Batch effects can alter or obscure the biological signal in the data [6, 7]. It is widely acknowledged that experimental design needs to take known sources of technical variation into consideration [8], as otherwise, the data can be biased beyond repair [9, 10]. Batch effects are pervasive to all high-throughput technologies, including next generation sequencing technologies and microarrays [11, 12]. A number of approaches for batch effect correction have been proposed [13 - 16] and their performance thoroughly evaluated [12, 17].

The fundamental objective of the batch effect adjustment procedure is to make all measurements of samples comparable for a meaningful biological analysis. Also, normalization aims to remove variance of non-biological origin [18]. Normalization adjusts only global properties of the measurements [6], while batch effects might affect specific groups of genes in different ways and persist even after normalization requiring additional correction procedures. Thus, normalization is only the first step in a technical biases correction workflow.

## The usage of proBatch in batch effect correction workflow
...
We recommend users to follow this batch correction workflow to ensure all measurements are comparable for downstream analysis. This workflow focuses on batch effect diagnostic and correction. Thus, it is assumed that the initial steps of pre-processing, such as peptide-spectrum matching, quantification, and FDR control are completed. We strongly suggest to perform the protein quantification after the batch effect correction, as this procedure alters the abundances of transitions and peptides, and these abundances are critical for protein quantity inference. We provide step-by-step illustrations to implement this workflow in the next sections of this vignette.  
...
## Loading the data 
The package typically requires three datasets: measurement data, peptide annotation and sample annotation.
