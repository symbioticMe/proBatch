...
vignette: >
  ...
  %\VignetteIndexEntry{proBatch in brief}
...
process is called normalization.  In `proBatch`, two normalization
...
Most of the batch effect correciton methods adjusts for feature-level biases [4]. 
...
Before the correction, samples from the same batch correlate better than between replicates. 
However, after the correction, the correlation between replicates becomes higher than the correlation
...
Here we show peptide correlation distribution generated from a complete dataset of the BXD mouse aging
study:
