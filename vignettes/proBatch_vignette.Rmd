---
title: "proBatch"
author: "Jelena Čuklina, Chloe H. Lee, Patrick Pedrioli, Ruedi Aebersold"
date: "`r Sys.Date()`"
#output: pdf_document
...
quality_col = 'm_score'
essential_columns = c(feature_id_col, measure_col, sample_id_col, quality_col)
```
...
Automatically imputed values (requants, imputed by OpenSWATH pipeline) can skew 
the distribution and bias the normalization and hence the measurements. 

Some peptides can have a lot of imputed values, that make their measurement unreliable:
```{r plot_with_requants1}
plot_single_feature('27827_IM(UniMod:35)FVGGPNTR_2', example_proteome, example_sample_annotation)
```
For some, imputed value are batch-specific:
```{r plot_with_requants2}
plot_single_feature('12295_TLDQVLEDVDQC(UniMod:4)C(UniMod:4)QALSQR_3', example_proteome, example_sample_annotation)
```


This is why we recommend to perform analysis with requant-free data matrix:
```{r clean_requants}
nrow(example_proteome)
example_proteome = clean_requants(example_proteome, example_sample_annotation,
                                  missing_frac_batch = .3, missing_frac_total = .3)
nrow(example_proteome)
```

And this matrix has imputed values removed:
```{r plot_without_requants1}
plot_single_feature('27827_IM(UniMod:35)FVGGPNTR_2', example_proteome, example_sample_annotation)
```

```{r plot_without_requants2}
plot_single_feature('12295_TLDQVLEDVDQC(UniMod:4)C(UniMod:4)QALSQR_3', example_proteome, example_sample_annotation)
```

Certain batch correction procedures, such as ComBat, will fail as they cannot 
infer the value for a peptide, when the whole batch is missing. We recommend to 
remove them when using such methods. 
```{r remove_peps_miss_batch}
length(example_proteome$peptide_group_label %>% unique())
example_proteome = remove_peptides_with_missing_batch(example_proteome, example_sample_annotation)
length(example_proteome$peptide_group_label %>% unique())
```
As you see, out of 535 original peptides, 94 had to be removed.
